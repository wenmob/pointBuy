  ...
  methods: {
    ...
    getSetTD () {
      return new Promise((resolve, reject) => {
        API.strategyListTD((res) => {
          console.log(res)
          if (res.code === 0) {
            this.setRule = res.data
            resolve()
          }
        })
      })
    ...
    buyVol () {
      ...
      this.algorithm(val, this.stockData)
    },
    // 发起策略的按钮
    sendTactics () {
      this.checkBuyNum()
      ...
      API.buyTAddD({stockCode: this.stockData.code, number: this.buyNum}, (res) => {
        if (res.code === 0) {
    ...
    // input失去焦点的验证
    checkBuyNum () {
      console.log(this.totalPrice + ',' + this.surplusFee)
      ...
      if (this.buyNum % 100 !== 0) {
        ...
        this.canSubmit = false
      } else if (this.totalPrice * 1 > this.surplusFee * 1) {
        Toast({
          message: '购买金额超过剩余金额',
          position: 'middle',
          duration: 3000
    ...
    algorithm (val, obj) {
      ...
      this.loss = (this.setRule.closeLine).toFixed(2)
      this.surplusFee = (this.setRule.balance).toFixed(2)
    }
  },
  watch: {
    stockData: {
